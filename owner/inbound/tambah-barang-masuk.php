...
if (isset($_POST['save'])) {
	...
	$koneksi->query("INSERT INTO inbound (po_inbound,id_item, in_kuantitas, in_tanggal, in_keterangan, kategori) VALUES ('$po_inbound','$id_item[$i]','$in_kuantitas[$i]','$in_tanggal','$in_keterangan','Barang Keperluan Toko')");
            ...
            $('#btn-tambah').on('click', function() {
                ...
                $('#tbl-barang-body').append(`
                    <tr required="">
					    ...
					    <?php
                        $ambil = $koneksi->query("SELECT * FROM keperluan_toko ORDER BY id_barang ASC");
                        while ($product = $ambil->fetch_assoc()) {
                        ?>
				        <option class="form-control"  value="<?php echo $product['id_barang'] ?>">
